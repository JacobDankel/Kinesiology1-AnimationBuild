...
UGoogleSTT* UGoogleSTT::CallSTT(TArray<uint8> CaptureData, FString language) {
	FGoogleSpeechRecognitionParams newParams;
	newParams.RawSamples = CaptureData;

	EGoogleSTTLanguage Elanguage;
	Elanguage = StringToEnum(language); //this is how we form the right enum from the string passed in

	newParams.Language = Elanguage;

	return GoogleSTT(newParams);
}

EGoogleSTTLanguage UGoogleSTT::StringToEnum(FString str) {
	// for every language we want to account for we must put under this if/else structure
	if (str == "es-MX") {
		return EGoogleSTTLanguage::SpanishMexico;
	}
	else {
		return EGoogleSTTLanguage::EnglishUnitedStates;
	}
}
...
void UGoogleSTT::OnResponse(FHttpRequestPtr Request, FHttpResponsePtr Response, bool WasSuccessful)
{
    if (!WasSuccessful)
    {
        ...
        if (Finished.IsBound())
        {
            Finished.Broadcast(TEXT(""), false, TEXT(""));
        }
        return;
    }
