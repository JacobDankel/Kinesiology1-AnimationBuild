...
public:
    ...
    UFUNCTION(BlueprintCallable, Category = "GoogleSpeech")
    static USoundWave* SoundWaveFromRawSamples(const TArray<uint8>& rawSamples, int32 sampleRate = 16000, int32 numChannels = 1, bool generateHeader = false);

    static FString GetApiKey();

	static UGoogleTTS* CallGoogleTTS(const FString& speech, FString lang);

	static UGoogleSTT* CallGoogleSTT(TArray<uint8> CaptureData, FString lang);
};
